...
public:
		...
		: myData()
	{
		...
		if (right.size() > 0)
			for (nodePtr temp = right.myData.myHead->next; temp != right.myData.myHead; temp = temp->next)
	...
	list& operator=(const list& right)
	{
		if (this != &right)
		{
			...
			for (nodePtr temp1 = right.myData.myHead->next, temp2 = myData.myHead->next; temp1 != right.myData.myHead; temp1 = temp1->next, temp2 = temp2->next)
				temp2->myVal = temp1->myVal;
		}
	...
	void push_front(const Ty& val)
	{
		...
		myData.myHead->next = new node;
		myData.myHead->next->myVal = val;
		myData.myHead->next->prev = myData.myHead;
		myData.myHead->next->next = temp;
		temp->prev = myData.myHead->next;
	...
	void push_back(const Ty& val)
	{
		...
		myData.myHead->prev->prev = temp;
		temp->next = myData.myHead->prev;
	...
	void clear() // erase all
	{
		...
		{
			for (nodePtr temp = myData.myHead->prev; temp != myData.myHead; temp = myData.myHead->prev)
				pop_back();
...
bool operator==(const list< Ty >& left, const list< Ty >& right)
{
	if (left.size() != right.size())
		return false;
	for (Ty* temp1 = left.myData.myHead->next, temp2 = right.myData.myHead->next; temp1 != left.myData.myHead; temp1 = temp1->next, temp2 = temp2->next)
		if (temp1->myVal != temp2->myVal)
			return false;
	return true;
}
