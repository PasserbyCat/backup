...
#include "Vector-1081447-Mid-1.h" // include definition of class vector 
...
HugeInteger< T > HugeInteger< T >::square()
{
   ...
   for (int i = 0; i < squareSize; i++) 
	   if (square.integer.begin()[i] > 9) {
		   square.integer.begin()[i + 1] += square.integer.begin()[i] / 10;
		   square.integer.begin()[i] %= 10;
	   }

   for (int i = squareSize - 1; square.integer.begin()[i] == 0 && i != 0; i--) {
	   square.integer.erase((square.integer.begin() + i));
   }//可能有問題
...
HugeInteger< T > HugeInteger< T >::squareRoot()
{
   ...
   unsigned int squaSize = integer.size();

   for (int i = sqrtSize - 1; i >= 0; i--) {
	   sqrt.integer.begin()[i]++;
	   while (1) {
		   if (sqrt.integer.begin()[i] == 10) {
			   sqrt.integer.begin()[i] = 9;
			   break;
		   }
		   HugeInteger squa(sqrt.square());
		   if (*this < squa) {
			   sqrt.integer.begin()[i]--;
			   break;
		   }
