...
public:
   ...
   iterator insert( const_iterator where, const char ch )
   {
      if( where >= myData.myPtr() && where <= myData.myPtr() + myData.mySize )
      {
		  ...
		  for (iterator it1 = myData.myPtr() + myData.mySize; it1 > myData.myPtr() + place; it1--) {
			  *it1 = *(it1 - 1);
		  }
		  myData.myPtr()[place] = ch;
		  myData.mySize = myData.mySize + 1;
      }
      else
         return nullptr;
   }
   ...
   iterator erase( const_iterator where )
   {
      if( where >= myData.myPtr() && where < myData.myPtr() + myData.mySize )
      {
		  for (size_type i = where - begin(); i < size(); i++) {
