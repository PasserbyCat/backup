...
public:
   ...
   iterator insert( const_iterator where, const value_type &val )
   {
      if( where >= myData.myFirst && where <= myData.myLast )
      {
		  ...
		  if (capacity() == 0) {
			  myData.myFirst = new value_type[1]();
			  myData.myEnd = myData.myFirst + 1;
		  ...
		  for (iterator it1 = myData.myLast; it1 > myData.myFirst + place; it1--) {
			  *it1 = *(it1 - 1);
		  }
...
bool operator==( const vector< Ty > &left, const vector< Ty > &right )
{
	...
	for (int i = left.size() - 1; i >= 0; i--)
		if (left.begin()[i] != right.begin()[i])//modify
			return false;
