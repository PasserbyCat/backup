...
public:
      ...
      : myData()
   {
      if( count != 0 )
      {
          myData.myFirst = new value_type[count]();//建一個動態矩陣(利用myFirst即myFirst一個指標且指向動態陣列的第一位)
          myData.myLast = myData.myEnd = myData.myFirst + count;//將myLast和myEnd指向該動態陣列的最後一位
      }
   }
   ...
   vector( const vector &right )
      : myData()
   {
       size_type temp = right.size();//temp即上面那個constructor function的count代表矩陣大小(同size)
       myData.myFirst = new value_type[temp]();//建一個動態矩陣(利用myFirst即myFirst一個指標且指向動態陣列的第一位)
       myData.myLast = myData.myEnd = myData.myFirst + temp;//將myLast和myEnd指向該動態陣列的最後一位

       for (size_type i = 0; i < temp; i++)
           *(myData.myFirst + i) = *(right.myData.myFirst + i);//將right陣列中的值存進新建的動態陣列中
   }
